 ...
 * Creator:     joe
 * Create-Date: 2011-4-27 上午10:48:07
 *******************************************************************************/
 ...
 * 
 * @author joe
 * @version $Id: PackageUtil.java 86 2011-05-05 02:03:49Z joe $
 */
public class PackageUtil {
    ...
    public static String[] getResourceInPackage(String packageName) throws IOException {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("PackageUtils: getResources: " + dirs + ", hasMoreElements:"
        ...
        while (dirs.hasMoreElements()) {
            ...
            if ("file".equals(protocol)) {
                getResourceInDirPackage(packageOnly, URLDecoder.decode(url.getFile(), "UTF-8"),
                    recursive, vResult);
    ...
    private static void getResourceInDirPackage(String packageName, String packagePath,
        ...
        for (File file : dirfiles) {
            if (file.isDirectory()) {
                getResourceInDirPackage(packageName + "." + file.getName(), file.getAbsolutePath(),
                    recursive, classes);
            } else {
    ...
    public static String[] findClassesInPackage(String packageName, List<String> included,
    ...
    private static void findClassesInDirPackage(String packageName, List<String> included,
                                                List<String> excluded, String packagePath,
                                                final boolean recursive, List<String> classes) {
        File dir = new File(packagePath);
        ...
        for (File file : dirfiles) {
            if (file.isDirectory()) {
                findClassesInDirPackage(packageName + "." + file.getName(), included, excluded,
